...
public class ComposeDiary {
    ...
    public void addDiary(String title, String content, int rating, String location,int interest) {
        //对日记进行writeIn
        ...
        String diaryName = currUser.getName()+"_"+diaryId+".txt";


        String diaryName_title = currUser.getName()+"_"+diaryId+"title.txt";
        //写入diaryName
        ...
        try(BufferedWriter writer = new BufferedWriter(new FileWriter("./diary_title/"+diaryName_title))) {
            writer.write(String.valueOf(interest));
            writer.write("\n");
            writer.write(title);
            writer.write("\n");
