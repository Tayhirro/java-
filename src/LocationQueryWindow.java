...
// 场所查询模块
public class LocationQueryWindow extends JFrame {

    public LocationQueryWindow() {
        setTitle("场所查询"); // 设置窗口标题
        setSize(200, 160); // 设置窗口大小
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); // 设置窗口关闭时的操作
        setLocationRelativeTo(null); // 将窗口置于屏幕中央

        JButton nearbyLocationsButton = new JButton("附近场所"); // 创建"附近场所"按钮
        JButton LocationQueryButton = new JButton("场所查询"); // 创建"场所查询"按钮
        ...
        // 添加附近场所按钮到面板
        panel.add(nearbyLocationsButton, gbc);

        // 添加场所查询按钮到面板
        gbc.gridy++;
        panel.add(LocationQueryButton, gbc);

        add(panel); // 将面板添加到窗口中

        // 为按钮添加 ActionListener
        nearbyLocationsButton.addActionListener(_ -> new NearbyLocationsFieldWindow().setVisible(true));
        LocationQueryButton.addActionListener(_ -> new LocationSearchFieldWindow().setVisible(true));
    }

    // 附近场所窗口
    static class NearbyLocationsFieldWindow extends JFrame {
        private final JTextField positionField; // 位置输入框
        private String position; // 位置字符串

        public NearbyLocationsFieldWindow() {
            ...
            submitButton.addActionListener(_ -> {
                ...
                LocationQuerySystem LocationQuerySystem = new LocationQuerySystem(); // 创建场所查询系统对象
                int positionId = LocationQuerySystem.getPointId(position); // 获取位置ID
                if(positionId == -1) {
                    JOptionPane.showMessageDialog(null, "当前位置不存在"); // 如果位置不存在，则弹出提示框
                    return;
                }
                String[] Locations = new String[1000]; // 创建场所数组
                double[] distances = new double[1000]; // 创建距离数组
                int num;
                if(straightDistanceRadioButton.isSelected()) { // 如果选择了直线距离
                    System.out.println("直线距离");
                    num = LocationQuerySystem.LocationQuery(positionId, "", true, Locations, distances); // 查询附近场所
                }else { // 如果选择了路径距离
                    System.out.println("路径距离");
                    num = LocationQuerySystem.LocationQuery(positionId, "", false, Locations, distances); // 查询附近场所
                }

                LocationDistanceWindow LocationDistanceWindow = new LocationDistanceWindow(Locations, distances,num); // 创建场所距离窗口
                LocationDistanceWindow.setVisible(true); // 显示场所距离窗口
            });
        }
    ...
    // 场所查询窗口
    static class LocationSearchFieldWindow  extends JFrame {
        private final JTextField positionField; // 位置输入框
        private final JTextField LocationSearchField; // 场所搜索输入框
        private final JRadioButton straightDistanceRadioButton; // 直线距离单选按钮

        private String position; // 位置字符串

        public LocationSearchFieldWindow() {
            ...
            // 创建UI组件
            positionField = new JTextField(20); // 创建位置输入框
            LocationSearchField = new JTextField(20); // 创建场所搜索输入框
            ...
            JPanel LocationSearchPanel = new JPanel(); // 创建场所搜索面板
            // 创建场所搜索面板并添加组件
            LocationSearchPanel.add(new JLabel("查询场所:")); // 添加标签
            LocationSearchPanel.add(LocationSearchField); // 添加场所搜索输入框
            ...
            add(LocationSearchPanel); // 添加场所搜索面板
            ...
            submitButton.addActionListener(_ -> {
                position = positionField.getText(); // 获取位置输入框的文本
                System.out.println("当前位置： " + position);
                String Location = LocationSearchField.getText(); // 获取场所搜索输入框的文本
                System.out.println("查询场所： " + Location);
                if(position.isEmpty()){
                    JOptionPane.showMessageDialog(null, "请输入当前位置"); // 如果位置为空，则弹出提示框
                    return;
                }
                ...
                if(straightDistanceRadioButton.isSelected()) { // 如果选择了直线距离
                    System.out.println("直线距离");
                    num = LocationQuerySystem.LocationQuery(positionId, Location, true, Locations, distances); // 查询场所
                }else { // 如果选择了路径距离
                    System.out.println("路径距离");
                    num = LocationQuerySystem.LocationQuery(positionId, Location, false, Locations, distances); // 查询场所
                }
...
// 场所距离窗口
class LocationDistanceWindow extends JFrame {

    public LocationDistanceWindow(String[] Locations, double[] distances,int num) {
        ...
        // 将数据添加到表格模型中
        for (int i = 0; i < num; i++) {
            tableModel.addRow(new Object[]{Locations[i], distances[i]}); // 添加行
        }
